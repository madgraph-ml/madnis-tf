...
channel_data = [([], [], [], []) for i in range(N_CHANNELS)]
for training_idx in range(args.ensemble_size):
################################
...
################################
    ...
    for i in range(N_CHANNELS):
        ...
        channel_data[i][0].append(p)
        channel_data[i][1].append(weight.numpy())
        channel_data[i][2].append(alphas.numpy())
        channel_data[i][3].append(alphas_prior)
...
for i, (ps, weights, alphas, alphas_prior) in enumerate(channel_data):
