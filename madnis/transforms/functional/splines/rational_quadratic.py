...
def unconstrained_rational_quadratic_spline(
    ...
    unnorm_divs = tf.dynamic_partition(unnormalized_derivatives, splittings, 2)
    ...
    outputs_inside, logabsdet_inside = rational_quadratic_spline(
        ...
        unnormalized_derivatives=unnorm_divs[1],
        inverse=inverse,
        left=left,
        right=right,
        bottom=bottom,
...
def rational_quadratic_spline(
    ...
    derivatives = (min_derivative + tf.nn.softplus(unnormalized_derivatives)) / (
        tf.cast(min_derivative + tf.math.log(2.0), tf.float64)
    )
    ...
    if inverse:
        a = (inputs - input_cumheights) * (
            input_derivatives + input_derivatives_p1 - 2 * input_delta
